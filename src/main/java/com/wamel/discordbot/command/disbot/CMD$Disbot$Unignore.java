...
public class CMD$Disbot$Unignore extends CMD$Disbot{
    ...
    public static void run(CommandSender sender, String[] args, String label) {
        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                Long id = DiscordBot.getDiscordId(uuid);
                ...
                sender.sendMessage("§c" + args[1] + "§6님은 이제 인증을 받아야만 서버 활동이 가능합니다.");

                if(Bukkit.getPlayer(uuid) != null) {
                    Player target = Bukkit.getPlayer(uuid);

                    MinecraftEvent.unLinkedPlayerList.add(target);
                    target.sendMessage("§6당신은 이제 인증을 받아야만 서버 활동이 가능합니다.");
                }
            } catch (Exception e) {
